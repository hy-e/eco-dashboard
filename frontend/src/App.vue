...
export default {
  ...
  computed: {
    doneRate() {
      if (!this.stats.total_ecos) return 0;
      return Math.round((this.stats.done_ecos / this.stats.total_ecos) * 100);
    },
  },
  created() {
    this.init();
  ...
  methods: {
    async init() {
      const [statsRes, seqRes] = await Promise.all([fetchStats(), fetchStepSeqs()]);
      this.stats = statsRes.data;
      this.stepSeqs = seqRes.data;
      await this.loadEcos();
    },
    async loadEcos() {
      const params = {};
    ...
    onFilter() {
      ...
      this.filterTimer = setTimeout(this.loadEcos, 300);
    },
    resetFilter() {
      this.filterEcoNo = '';
      this.filterStepSeq = '';
...
body { background: #111520; color: #c8ccd8; font-family: -apple-system, 'Segoe UI', sans-serif; font-size: 14px; }
...
.progress-fill {
  height: 100%;
  background: linear-gradient(90deg, #3ecf8e, #4f8ef7);
  border-radius: 4px;
...
.filter-group select:focus { border-color: #4f8ef7; }
